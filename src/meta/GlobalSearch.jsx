...
const sources = [
  ...
  { table: 'saved_items', text: (row) => `${row.title} ${row.content ?? ''} ${row.tags?.join(' ') ?? ''}` },
...
export default function GlobalSearch({ onOpen }) {
  ...
  return <main className="tracker-shell"><section className="tracker-card"><header><span className="eyebrow">Across your data</span><h1>Global search</h1></header><input className="global-search" autoFocus placeholder="Search entries, notes, and tasks" value={query} onChange={(event) => setQuery(event.target.value)} /><label className="search-filter">Engine<select value={engineFilter} onChange={(event) => { setEngineFilter(event.target.value); setModuleFilter('all'); }}><option value="all">All engines</option>{Object.entries(engines).map(([id, title]) => <option key={id} value={id}>{title}</option>)}</select></label><label className="search-filter">Module<select value={moduleFilter} onChange={(event) => setModuleFilter(event.target.value)}><option value="all">All modules</option>{moduleOptions.map((module) => <option key={module.id} value={module.id}>{module.title}</option>)}</select></label>{status && <p className="status">{status}</p>}<section className="recent"><h2>{visible.length ? `${visible.length} results` : 'Search your records'}</h2><ul>{visible.map((item) => <li key={`${item.table}-${item.id}`}><button className="result" onClick={() => item.module_id && onOpen(item.module_id)}><span>{getModule(item.module_id)?.title ?? 'Goals'}</span><small>{item.title ?? item.note ?? JSON.stringify(item.fields ?? item.metadata)}</small></button></li>)}</ul></section></section></main>;
}
