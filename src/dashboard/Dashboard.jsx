...
export default function Dashboard({ onOpen }) {
  ...
  useEffect(() => { getAnonymousUser().then(async (user) => { const tables = [...new Set(Object.values(tableByEngine))]; const results = await Promise.all(tables.map(async (table) => { const { data, error } = await supabase.from(table).select('*').eq('user_id', user.id); if (error) throw error; return [table, data ?? []]; })); const next = Object.fromEntries(results); const due = next.due_items ?? []; const checkins = next.streak_checkins ?? []; const lists = next.checklist_items ?? []; setByTable(next); setActions({ overdue: due.filter((row) => active(row) && row.due_at && row.due_at < today).sort((a, b) => a.due_at.localeCompare(b.due_at)).slice(0, 5), upcoming: due.filter((row) => active(row) && row.due_at && row.due_at >= today).sort((a, b) => a.due_at.localeCompare(b.due_at)).slice(0, 5), activeLists: lists.filter(active).length, habits: checkins.filter((row) => row.completed_on === today).length }); }).catch((error) => setStatus(error.message)); }, []);
